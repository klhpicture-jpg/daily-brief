...
6. **First run.** Run the "digest" workflow with `dry_run` checked, open the run log and read the message it would have sent. Then run it unchecked. The scheduled run is 18:25 Europe/Copenhagen daily, year round (two UTC cron slots, a step lets only the one that is 18 local through). The message lands around 18:28. Only the digest itself rings the phone; failure and test messages arrive silently.
